class RegisterController {
  ...
  findById(db, req, res) {
    const { id } = req.params;
    db.each(`SELECT * FROM register WHERE id = ${id}`, (err, rows) => {
      try {
        res.json(rows);
      } catch {
        console.error(err);
  ...
  store(db, req, res) {
    const { name, birth, nationality, gender, email } = req.body;

    const stmt = db.prepare(
      `INSERT INTO register (name, birth, nationality, gender, email) VALUES(?, ?, ?, ?, ?)`
    );
  ...
  update(db, req, res) {
    ...
    const { id } = req.params;
    const stmt = db.prepare(
      `UPDATE register SET name = ?, birth = ?, nationality = ?, gender = ?, email = ? WHERE id = ?`
    );
